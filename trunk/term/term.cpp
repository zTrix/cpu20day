...
struct Tconsole
{
	...
	void run_L(SOCKET &client, char *argv[],HANDLE& com,int isSIM);    //L命令
...
void Tconsole::run_D(SOCKET &client,char* argv[64],HANDLE& com,int isSIM )
{
	int len=0;
	char ch;	
	short count=0,first_addr=0,num=0,temp=0;  //读取次数、首地址、内存中的数,临时用
	DWORD size;
	first_addr = get_value(argv[1]);
	if(first_addr == 0xFFFF || first_addr>=0x6000 || first_addr<0x4000) 
		first_addr = 0x4000;
...
void Tconsole::run_L(SOCKET &client,char* argv[64],HANDLE& com,int isSIM )
{
	char ch;
	DWORD size;
	unsigned short addr=0x4000,temp=0,len=0;
	addr = get_value(argv[2]);
	if (addr == 0xFFFF || addr >= 0x6000 || addr < 0x4000) 
		addr = 0x4000;
	...
	if(isSIM==1){	
		...
		while(index * 2 < length)
		{
			...
			size=0;
			while(size==0)		
				WriteFile(com,&buffer[index*2],1,&size,NULL);

			size=0;
			while(size==0)		
				WriteFile(com,&buffer[index*2+1],1,&size,NULL);
			addr=addr+1;		
			index ++;
		}	
						ch=0;
			...
			switch(len = recv(client,&ch,1,0))
			{
				...
				default:
					switch(ch)
					{
						...
						printf("Elapsed time: %.3f secs.\n", (float)1.0 * (end - start) / CLOCKS_PER_SEC);
							return;					
						default:
							printf("%",ch);
							break;
